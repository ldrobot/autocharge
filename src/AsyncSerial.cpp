...
#include "tjc_screen/AsyncSerial.h"    

#include <string>   //包含一系列头文件
#include <algorithm>
#include <thread>
#include <mutex>
...
using namespace boost;   //用关键字namespace去定义命名空间

//
//Class AsyncSerial    //类的名字，class是指类
//

#ifndef __APPLE__


 //以此类推，下面定义的都是成员变量 boost;std;bool是变量的类型。
    ...
    std::function<void (const char*, size_t)> callback;  //定义回调函数（相当于中断服务函数）
};

AsyncSerial::AsyncSerial(): pimpl(new AsyncSerialImpl)  //空函数，不执行什么操作
{

}

AsyncSerial::AsyncSerial(const std::string& devname, unsigned int baud_rate,   //定义异步串口函数里的奇偶校验，波特率，字节大小等参数
...
void AsyncSerial::open(const std::string& devname, unsigned int baud_rate,  //这个函数的功能是设置好Io,波特率，奇偶校验后打开串口通信
        asio::serial_port_base::parity opt_parity,
        asio::serial_port_base::character_size opt_csize,
        asio::serial_port_base::flow_control opt_flow,
        asio::serial_port_base::stop_bits opt_stop)
{
    if(isOpen()) close();   //判断是否已经打开串口，如果打开了就关闭，然后设置下面的参数

    setErrorStatus(true);//If an exception is thrown, error_ remains true  //设置ErrorStatus标志为真
    ...
    //This gives some work to the io_service before it is started  //这几句涉及到ros里的服务（service），关于io口的service
    pimpl->io.post(boost::bind(&AsyncSerial::doRead, this));

    thread t(boost::bind(&asio::io_service::run, &pimpl->io));
    pimpl->backgroundThread.swap(t);
    setErrorStatus(false);//If we get here, no error
    pimpl->open=true; //Port is now open  //最后再打开端口
}

bool AsyncSerial::isOpen() const  //bool类型函数，判断是否打开
{
    return pimpl->open;
}

bool AsyncSerial::errorStatus() const       //判断errorStatus真假
{
    lock_guard<mutex> l(pimpl->errorMutex);
    return pimpl->error;
}

void AsyncSerial::close()    //AsyncSerial类中的串口关闭成员函数
{
    if(!isOpen()) return;
...
void AsyncSerial::write(const char *data, size_t size)   //串口写函数，写数据
{
    {
...
void AsyncSerial::write(const std::vector<char>& data)  //串口写函数，写向量
{
    {
        lock_guard<mutex> l(pimpl->writeQueueMutex);
...
void AsyncSerial::writeString(const std::string& s)//串口写函数，写字符串
{
    {
        lock_guard<mutex> l(pimpl->writeQueueMutex);
...
AsyncSerial::~AsyncSerial()   //取反函数，开则关，关则开
{
    if(isOpen())
    {
        try {
...
void AsyncSerial::doRead()  //异步通信读函数
{
...
void AsyncSerial::readEnd(const boost::system::error_code& error,  //判断读操作是否完成
...
void AsyncSerial::doWrite()     //异步通信的底层驱动函数
                                                   //最后几个函数是callback回调函数
{
...
void CallbackAsyncSerial::setCallback(consst std::function<void (const char*, size_t)>& callback)
{
